...
_THRESHOLD = 0.05
_GAUSS_K   = np.sqrt(-2.0 * np.log(_THRESHOLD))


def build_vertical_lut(zmin=80.0, zmax=300.0, nz=256,
    #Lazarev electron energy range 
    E_min_keV  =  1.0,   # keV — raise to push green band lower
    E_max_keV  = 20.0,   # keV — lower to raise and soften peak
    n_energies =  8,     # energy samples — more = smoother curve

    GREEN_START=100.0, GREEN_PEAK=138.0, GREEN_END=260.0, GREEN_AMP=1.00,
    BLUE_START=100.0,  BLUE_PEAK=138.0,  BLUE_END=249.0,  BLUE_AMP=0.50,
    RED_START=165.0,   RED_PEAK=270.0,   RED_TOP_VAL=0.15, RED_AMP=0.30,
):
    z = np.linspace(zmin, zmax, nz, dtype=np.float32)

    def gauss_curve(z, start, peak, end, amp):
        sl = (peak - start) / _GAUSS_K
        sh = (end  - peak)  / _GAUSS_K
    ...
    sl = (RED_PEAK - RED_START) / _GAUSS_K
    ...
    # Lazarev: physically-based intensity profile 
...
def _perlin_grad1d(h, x):
    """1D gradient: hash selects +1 or -1 slope."""
...
def perlin_1d(x, seed=0):
    ...
    perm = np.concatenate([perm, perm])   # wrap

    xi   = np.floor(x).astype(np.int32) & 255
    xf   = x - np.floor(x)
    u    = _perlin_fade(xf)

    g0   = _perlin_grad1d(perm[xi],     xf)
    g1   = _perlin_grad1d(perm[xi + 1], xf - 1.0)
    return g0 + u * (g1 - g0)          # lerp
...
def build_tao_noises(s_map, XX, YY, L):
    ...
    N1_FREQ = 2.5 # cycles per curtain length — high frequency
    N1_SEED = 42
    # noise 2: low freq 1D perturbs color LUT z
    N2_AMP = 18.0 # km  — how much color shifts along strip
    N2_FREQ = 1.5 # cycles per curtain — low frequency
    N2_SEED = 137
...
def probability_tables(footprint_xy, A_lut, xy_power=1.5, z_power=1.0):
    ...
    return (
        pdf_xy.astype(np.float32),
        cdf_xy.astype(np.float32),
        pdf_z.astype(np.float32),
        cdf_z.astype(np.float32),
    )
def smoothstep01(x):
    x = np.clip(x, 0.0, 1.0)
...
def bake_volume(
    ...
    z_shift_xy = (
        2.0 * np.sin(2.0 * np.pi * 0.45 * s01 + 0.3) +
        1.0 * np.sin(2.0 * np.pi * 1.0  * s01 + 1.1)
    ).astype(np.float32)

    height_scale_xy = (
        1.0
        + 0.08 * np.sin(2.0 * np.pi * 0.6 * s01 + 0.8)
        + 0.04 * np.sin(2.0 * np.pi * 1.2 * s01 + 2.0)
    ).astype(np.float32)
    ...
    pdf_xy, cdf_xy, pdf_z, cdf_z = probability_tables(
        footprint_xy, A_lut, xy_power=1.5, z_power=1.0
    )
